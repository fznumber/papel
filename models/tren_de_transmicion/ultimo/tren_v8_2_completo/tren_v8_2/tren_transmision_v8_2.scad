/* ==========================================================
   TREN DE TRANSMISIÓN - EXTRUSOR DE DOBLE TORNILLO (V8.2 FIX)
   ----------------------------------------------------------
   Cambios respecto a V8.1 (verificados con boolean intersection,
   no solo revisión visual):
   1. [CRÍTICO] p_pillar en las 2 posiciones frontales (-60,-43 y
      60,-43) colisionaba físicamente con p_spacer_base_front:
      5771 mm3 de solape confirmado, mismo XY y mismo rango Z
      (10 a 44). Fix: se creó p_pillar_short(), que solo cubre
      desde el tope del bloque (Z_stage2=44) hasta la placa motor
      (Z_motor_pl=100), es decir 56mm en vez de 90mm, arrancando
      en Z=44 en vez de Z=10. Se usa SOLO en esas 2 posiciones;
      las otras 4 pillar_coords siguen usando p_pillar() normal.
   2. [CRÍTICO] La holgura entre gear2 (verde) y el tope de
      p_spacer_base_front se había perdido: el bloque llegaba
      exactamente a Z_stage2, tocando el engranaje (0mm de gap,
      confirmado con intersection = cara degenerada). Fix: se
      reintrodujo la variable front_spacer_clearance = 4, restada
      de la altura del bloque, igual que en la corrección anterior.
   ========================================================== */
...
module p_gear2() { 
    difference() {
        ...
        hex_hole(hex_shaft_flat, 60); // Incrementado para cruzar todo
...
if (part == "assembly") {
    ...
    color("white") {
        ...
        // en Z_stage2 (tope del bloque), NO en Z=10 (ya no se solapan).
        translate([-60, -43, Z_stage2]) p_pillar_short(); translate([60, -43, Z_stage2]) p_pillar_short();
