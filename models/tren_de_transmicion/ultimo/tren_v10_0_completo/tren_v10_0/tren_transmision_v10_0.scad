/* ==========================================================
   TREN DE TRANSMISIÓN - EXTRUSOR DE DOBLE TORNILLO (V10.0)
   ----------------------------------------------------------
   Historial reciente:
   V8.2/V8.3: fix colisión pilar/bloque + holgura gear2.
   V9.0: eje hexagonal -> varilla cuadrada 3/8".
   V10.0 (este archivo):
   - Rodamiento 6002ZZ (17x32x9... perdón, 15x32x9) reemplazado
     por 6703 (17x23x5, serie extra-delgada). Motivo: se confirmó
     con corte transversal real que los bolsillos de pos_L y
     pos_R (separados solo 25.3mm, distancia fija entre tornillos)
     se fusionaban en una sola cavidad con el 6002ZZ (1800mm3 de
     solape medido). El 6703 deja 2.3mm de pared sólida real.
   - Efecto secundario positivo: el poste del bearing_adapter
     mejora de 0.45mm a 1.45mm de pared alrededor del eje
     cuadrado (sigue siendo el punto más ajustado del tren --
     imprimir con cuidado ahí, 6+ perímetros).
   - bearing_w bajó de 9mm a 5mm (ancho real del 6703). Se
     corrigieron 3 offsets que antes estaban escritos a mano
     como "9" en la vista de ensamblaje y quedaban desalineados;
     ahora usan la variable bearing_w.
   ========================================================== */
...
// FIX V10.0: cambio de rodamiento 6002ZZ -> 6703 (17x23x5mm),
// serie "extra-delgada". Resuelve el solape confirmado entre los
// bolsillos de pos_L y pos_R (que con el 6002ZZ se fusionaban en
// una sola cavidad, 1800mm3 de solape medido). Con 6703 queda
// 2.3mm de pared solida real entre ambos. Efecto secundario: el
// poste del adaptador sube de 0.45mm a 1.45mm de pared (mejora,
// aunque sigue siendo la zona mas ajustada del tren -- imprimir
// con cuidado, 6+ perimetros, ahi especificamente).
bearing_od = 23.05;       // 6703: OD nominal 23mm + 0.05 (mismo criterio que antes)
bearing_id = 16.8;        // 6703: ID nominal 17mm - 0.2 (post con interferencia leve)
bearing_w = 5;             // 6703: ancho nominal 5mm (antes 9mm -- el rodamiento es mas angosto)
